...
class SmartShopApp {
  ...
  attachEventListeners() {
    ...
    [this.priceRange, this.minRating, this.sortBy, this.inStock].forEach(
      (element) => {
  ...
  applyFiltersAndDisplay() {
    const filters = {
      maxPrice: this.priceRange.value
        ? parseFloat(this.priceRange.value)
        : null,
  ...
  normalizeProduct(product, index = 0) {
    ...
    return {
      ...
      storeUrl: product?.storeUrl || "",
      productUrl:
        product?.productUrl || product?.link || product?.storeUrl || "",
      image:
        product?.image ||
  ...
  clearWishlist() {
    if (
      confirm("Sigur vrei să ștergi toate produsele din lista de favorite?")
    ) {
      this.wishlist = [];
      this.saveWishlist();
      this.hideModal();
